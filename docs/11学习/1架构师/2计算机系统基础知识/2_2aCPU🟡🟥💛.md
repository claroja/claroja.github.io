...
- 运算器和控制器的组件
    - CPU的两个组成部分是`运算器`和`控制器`
    - 运算器的组件包含哪些
        - 算术逻辑单元(Arithmetic Logic Unit, ALU)的作用: `计算`
        - 累加寄存器(Accumulator Register，AR)的作用: `暂存数据`
    ...
    - 控制器的组件包含哪些
        ...
        - 程序计数器(Program Counter，PC)的作用: `下一条`指令
        - 地址寄存器(Address Register，AR)的作用: `数据`地址
        - 指令译码器(Instruction Decoder，ID)的作用: `解释`
...
1. 运算器: 算术运算操作
    ...
    2. 运算器的组件:
        ...
        2. 累加寄存器(Accumulator Register，AR)：通用寄存器，为ALU提供一个工作区，用于`暂存数据`
