...
- 💛给定关系R(A，B，C，D)和S(A，C，E，F)，以下______与$\sigma_{R.B > S.E}(R \bowtie S)$等价。
...
- 💚若关系R、S如下图所示，则关系R与S进行自然连接运算后的元组个数和属性列数分别为(  )；关系代数表达式$\pi_{1,4}(\sigma_{3 = 6}(R\times S))$与关系代数表达式(  )等价。
...
- 💚给定关系R(A，B，C，D，E)与S(A，B，C，F，G)，那么与表达式$\pi_{1, 2, 4, 6, 7}(\sigma_{1<6}(R \Join S))$等价的SQL语句如下：SELECT(  )FROM R, S WHERE(  );
