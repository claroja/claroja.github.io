...
- 维度1: 与外界作用方式: I`(Introversion)` or E`(Extroversion)`
- 维度2: 获取信息方式: S`(Sensing)` or N`(Intuition)`
- 维度3: 决策方式: T`(Thinking)` or F`(Feeling)`
- 维度4: 执行方式: J`(Judging)` or P`(Perceiving)`

## MBTI概念

MBTI是“Myers-Briggs Type Indicator” 的缩写，即迈尔斯-布里格斯类型指标。MBTI以瑞士心理学家卡尔・荣格（Carl Jung）的心理学理论为基础。荣格提出，人的心理活动有四种基本功能：感觉（Sensing）、直觉（Intuition）、思维（Thinking）和情感（Feeling），以及两种态度类型：内倾（Introversion）和外倾（Extraversion）。MBTI 在这个基础上进一步发展，增加了J(Judging)和P(Perceiving). 共四个维度.
...
## 维度2: 获取信息方式: S(Sensing, 实感, ??) or N(Intuition, 直觉, 想象推理)
...
🌰对于桌子的长度, N人凭感觉, 大概有半米; S人拿尺子直接测量, 600mm

## 维度3: 决策方式: T(Thinking, 理性) or F(Feeling, 感性)
...
🌰决策过程, F人更关注他人的感受;T人更关注事情的本身.

## 维度4: 执行方式: J(Judging, 判断, 有条理规划) or P(Perceiving, 感知, 无条理随性)
