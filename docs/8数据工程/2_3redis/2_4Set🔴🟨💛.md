...
- 所有命令均以`()`开头, key代表的是某一Set的`()`, value代表的向Set里添加的值
...
- 💛点赞
    ...
    2. uid:1取消了对article:1文章点赞: `()`
    3. 获取article:1文章所有点赞用户: `()`
    4. 获取article:1文章的点赞用户数量: `()`
    5. 判断用户uid:1是否对文章article:1点赞了: `()`
...
- 💛抽奖活动
    ...
    1. key为抽奖活动名，value为员工名称，把所有员工名称放入抽奖箱：`()`
    2. 允许重复抽奖
        1. 抽取1个一等奖：`()`
        2. 抽取2个二等奖: `()`
        3. 抽取3个三等奖: `()`
...
- 所有命令均以`S`开头, key代表的是某一Set的`名字`, value代表的向Set里添加的值
- 点赞
