...
在微服务中，OATUH2的TOKEN处理方案有多种

- 有通过统一网关加工处理， 其他服务不参与， 直接通过网关获取用户信息， 但网关职能变得就不再单一， 且其他服务安全性薄弱；
- 有通过JWT TOKEN方式， 各服务直接从JWT中获取用户信息， 但附带信息有限， 且要加解密处理，占用计算成本， 传递JWT的长度比之前token增加数倍；
- 还有方案是， 各服务直接拿token远程调用认证服务， 获取用户信息，远程连接开销非常高， 且增加认证服务的压力。
