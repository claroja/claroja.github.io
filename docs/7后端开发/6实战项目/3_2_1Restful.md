...
Rest是一种软件架构与设计风格，只提供了一些原则与约定条件。RESTful提供了一组架构约束，当作为一个整体服务来应用时，强调组件交互的可伸缩性、接口的通用性、组件的独⽴部署、以及用来减少交互延迟、增强安全性、封装遗留系统的中间组件。满足这些约束条件和原则的应用程序或设计就是Restful。
...
- Level 2: HTTP Verbs
    ...
    Web服务使用不同的 HTTP 方法来进行不同的操作，并且使用 HTTP 状态码来表示不同的结果。等级2加入了HTTP方法处理：
...
- Level 3: Hypermedia Controls
    ...
    Web服务使用 HATEOAS。在资源的表达中包含了链接信息。客户端可以根据链接来发现可以执行的动作。等级3为超媒体控制(HATEOAS)， 也是最为成熟的Rest模型。
